...
const Slider = ({setpost}) => {
    ...
    const rightClick=()=>{
        setTranslation((currentTranslation)=>{
            ...
            if (newTranslation ===-330*6){
                setIndex((current)=>current===10?1:current+1)
                console.log(index)
                setRightButtonStyle({cursor: `default`, backgroundColor:`red`})
                let clone = [];
                let clone2 = [];
                ...
                for (let key in cards) {
                    clone2[key] = cards[key];
                }
                /*setCards([clone.filter((item)=>item.id>index), clone2.filter((item)=>item.id<index)]);*/
                setCards(clone.filter((item)=>item.id>index));
                console.log({...cards})
            }
            if (newTranslation !==0){
                setLeftButtonStyle({cursor: `pointer`, backgroundColor:`#FE8700`})
            }
